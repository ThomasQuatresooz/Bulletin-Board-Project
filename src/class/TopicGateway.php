...
class TopicGateway
{
    ...
    public function findMostRecentByBoardId($id) :?iterable
    {
        try {
            ...
            $sth = $db->prepare('SELECT topics.* FROM messages INNER JOIN topics ON messages.topics_idtopics = topics.idtopics WHERE topics.boards_idboards = ? GROUP BY topics.idtopics ORDER BY `messages`.`date-creation` DESC LIMIT 3');
